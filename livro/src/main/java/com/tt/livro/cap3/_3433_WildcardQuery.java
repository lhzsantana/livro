...
public class _3433_WildcardQuery extends _0_Abstract {

	public _3433_WildcardQuery() {
		_211_Connection connection = new _211_Connection();

		client = connection.createClient();
	}

	public void searchWildcardQuery(String tituloWithWildcard) {
				
		SearchResponse response = client.prepareSearch(index).setTypes(type)
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(
						QueryBuilders.wildcardQuery("titulo", tituloWithWildcard)
				)
				.execute()
				.actionGet();
