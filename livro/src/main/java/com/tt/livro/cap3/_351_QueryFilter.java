...
public class _351_QueryFilter extends _0_Abstract {

	public _351_QueryFilter() {
		_211_Connection connection = new _211_Connection();

		client = connection.createClient();
	}

	public void searchQueryFilter(String query) {
		
		SearchResponse response = client.prepareSearch(index).setTypes(type)
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(QueryBuilders.matchAllQuery())
				.setPostFilter(
						FilterBuilders.queryFilter(QueryBuilders.termQuery("cidade",query))
				)
				.execute()
				.actionGet();
